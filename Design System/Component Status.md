...
| Photo Row | ✅ Built | `2259:4198` | 4 variants (`Tile` M/L × `Inset` Screen/Column) + 7 boolean slots. Full-bleed or column-inset photo strip carrying the iOS context-menu lift headroom ([[Rules#L6]]). Published 2026-09-17; **16 instances** across Receipts, Truck Service, Deposit Slip, Adjustments, Collect Payment. `Claims · Filled Information` deliberately not swapped — its add tile is a [[Components/Button\|Button]], which this component doesn't model yet. See [[Components/Photo Row\|Photo Row]]. |
| Photo Grid | 📋 Planned | — | Wrap grid of [[Components/Content\|Content]] tiles (112pt + caption + status badge) for inspection photos — BOL, Inspection Review, Inspection (Splits), Customer Review, Vehicle Details. Needs the same `Space/S` vertical inset as [[Components/Photo Row\|Photo Row]] ([[Rules#L6]]); the rule is applied by hand on those screens today. Filed 2026-09-17. |
...
| Segment Control | ✅ Built | `321:1008` | 13 combinations (Selected × Segments × Scrollable). iOS-style segmented control. ⚠️ Hidden trailing `xMark` slot **scheduled for deletion** from all 13 variants (ratified 2026-08-11, [[Rules#C5]]) — Figma edit + republish pending. See [[Components/Segment Control\|Segment Control]]. |
...
| Navigation Bar | ⚠️ Gap | `309:2318` | 6 Type variants (Default, Search Field, Order Label, Settings, Stack, Horizontal). **Missing: a text trailing action** (`Reset` / `Clear` / `Done`) — every variant's trailing slot is an 88pt container holding a 44pt circular *icon* button, so [[Order Search Screen]] had to override off-system. Needs a new variant or a text-trailing property. Filed 2026-08-11 per [[Rules]] C3. Composes [[Components/Button\|Button]] + [[Components/Title and Controls\|Title and Controls]] + [[Components/Select\|Select]] + [[Components/Badge\|Badge]]. See [[Components/Navigation Bar\|Navigation Bar]]. |

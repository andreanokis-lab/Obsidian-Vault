...
- **Cross-screen, 2026-09-17** — context-menu headroom on every photo container: **35 containers across ~21 screens** on the `DS` page given the [[Photo Row]] insets (`Space/S` vertical; `Space/L` horizontal where full-bleed) after engineering reported photo tiles clipping on long-press. Three families: photo strips (Truck Service · Multiple Photos `251:38601`, Collect Payment `1:30948`, Receipts ×5, Deposit Slip, Claims, Adjustments ×2, Adjustment (Done)); single tiles (Truck Service · Add Truck Services, Sheet, Photo Actions, Receipts · Accepted Receipt, Deleting Image (Long Press)); and the [[Content]]-based inspection grids (BOL ×2, Customer Review ×2, Inspection (Splits) ×6, Inspection Review ×6, Vehicle Details ×2 — the Vehicle Details `photo grid` and its `cell` frames were clipping their own tiles and are now unclipped). Excluded: the vertical camera film strips (`Image View` `1189:31937`, `Take a photo` `645:20663`) and the already-lifted photo on Deleting Image (`186:15458`). Rule: [[Rules#L6]] · component: [[Photo Row]] — published and **16 containers swapped to instances** (`Inset=Screen` full-bleed ×2, `Inset=Column` ×14); `Claims · Filled Information` kept hand-built because its add tile is a Button; the Content-based inspection grids keep the insets by hand until `Photo Grid` exists.
